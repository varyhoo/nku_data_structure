...
public:
	...
	void Insert(const int&,const T&v);
	bool Search(const T&);
	void Delete(const T&);
	void Output();
	int divisor;
...
template<class T>
void Chain_HashTable<T>::Insert(const int& x,const T&v)
{
	if (Search(x))
	{
		cout << "元素已存在" << endl;
...
bool Chain_HashTable<T>::Search(const T& x)
{
	...
	if (!p)return false;
	while (p->key != x && p->key != INT_MAX)p = p->next;
...
void Chain_HashTable<T>::Output()
{
	...
	for (int i = 0; i < divisor; i++)
	{
		...
		if (!p)continue;
		if (p->key == INT_MAX)continue;
		cout << "元素位于第" << i << "个桶中,key为：";
		while (p->key != INT_MAX)
		{
			cout << p->key << " "<<"value为："<<p->data;
			p = p->next;
		}
		cout << endl;
	}
