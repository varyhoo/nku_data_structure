...
/*
对于左孩子的情况：
如果 other 是 parent 的右孩子，且 other 的颜色是红色，表示出现了 Lr 型的情况。
根据 other 的左右孩子的颜色情况，分别进行相应的旋转和颜色调整。
对于右孩子的情况：
如果 other 是 parent 的左孩子，且 other 的颜色是红色，表示出现了 Rr 型的情况。
根据 other 的左右孩子的颜色情况，分别进行相应的旋转和颜色调整。
循环结束后，对删除的节点进行颜色调整，确保其颜色为黑色。
*/
	//类比AVL的删除操作
