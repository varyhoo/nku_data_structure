...
public:
	...
	void Split(DoubleLinkedList& a,DoubleLinkedList& b);
...
template<class T>
void DoubleLinkedList<T>::Split(DoubleLinkedList& a, DoubleLinkedList& b )
{
	int i;
	for (i = 1; i <= this->Getlength() - 1; i += 2)
	{
		a.PushBack(this->GetNowNode());
		b.PushBack(this->GetNowNode());
	}
	if (i <= this->Getlength())
	{
		a.PushBack(this->GetNowNode());
